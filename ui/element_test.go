...
func TestSfntBuildSceneButtonText(t *testing.T) {
	...
	total := len(scene.TexturedGlyphs) + len(scene.MSDFGlyphs)
	if total != 2 {
		t.Errorf("Button label 'OK' should produce 2 glyphs, got %d", total)
	}
}
...
func TestSfntBuildSceneColumnTextAndButtonText(t *testing.T) {
	...
	total := len(scene.TexturedGlyphs) + len(scene.MSDFGlyphs)
	if total != 7 {
		t.Errorf("expected 7 glyphs, got %d", total)
	}
...
func TestSfntCheckboxWithLabel(t *testing.T) {
	...
	// Should have glyphs for both the checkmark and label.
	total := len(scene.TexturedGlyphs) + len(scene.MSDFGlyphs)
	if total == 0 {
		t.Fatal("Sfnt Checkbox should produce TexturedGlyphs or MSDFGlyphs")
	}
}
