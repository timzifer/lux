...
func layoutFlex(node flexElement, area bounds, canvas draw.Canvas, tokens theme.TokenSet, hitMap *hit.Map, hover *HoverState, focus *FocusState) bounds {
	n := len(node.Children)
	if n == 0 {
	...
	for i, child := range node.Children {
		if exp, ok := child.(expandedElement); ok {
			...
			cb := layoutElement(child, area, nc, tokens, nil, nil)
			if isRow {
	...
	for i := range infos {
		if infos[i].expanded {
			...
			cb := layoutElement(exp.Child, measureArea, nc, tokens, nil, nil)
			if isRow {
	...
	for i, child := range node.Children {
		...
		cb := layoutElement(actualChild, childArea, canvas, tokens, hitMap, hover, focus)
