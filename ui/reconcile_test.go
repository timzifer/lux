...
func TestReconcileWidgetStatePreserved(t *testing.T) {
	...
	_, _ = r.Reconcile(tree, th, noopSend, nil, nil, "", nil)
	if r.StateCount() != 1 {
...
func TestReconcileStateResetOnKeyChange(t *testing.T) {
	...
	r.Reconcile(tree1, th, noopSend, nil, nil, "", nil) // RenderCount = 2
...
func TestReconcileNoChangeReturnsFalse(t *testing.T) {
	...
	_, changed := r.Reconcile(tree, th, noopSend, nil, nil, "", nil)
	if !changed {
...
func TestReconcileDetectsTextChange(t *testing.T) {
	...
	r.Reconcile(Text("v1"), th, noopSend, nil, nil, "", nil)
	_, changed := r.Reconcile(Text("v2"), th, noopSend, nil, nil, "", nil)
	if !changed {
...
func TestReconcileDetectsStructuralChange(t *testing.T) {
	...
	_, changed := r.Reconcile(Column(Text("a"), Text("b")), th, noopSend, nil, nil, "", nil)
	if !changed {
...
func TestReconcileExpandsWidgetToElement(t *testing.T) {
	...
	resolved, _ := r.Reconcile(tree, th, noopSend, nil, nil, "", nil)
...
func TestReconcileRegistersFocusableWidgets(t *testing.T) {
	...
	r.Reconcile(tree, th, noopSend, nil, fm, "", nil)
...
func TestEquatableReRendersWhenNotEqual(t *testing.T) {
	...
	r.Reconcile(ComponentWithKey("eq", eqWidget{Label: "v1"}), th, noopSend, nil, nil, "", nil)
	uid := MakeUID(0, "eq", 0)

	// Change label — Equatable.Equal should return false, Render called again.
	r.Reconcile(ComponentWithKey("eq", eqWidget{Label: "v2"}), th, noopSend, nil, nil, "", nil)
	s := r.StateFor(uid).(*counterState)
	if s.RenderCount != 2 {
