...
- [ ] `input.KeyMsg` auf typisierten `Key uint32` + `ModifierSet` Bitfield umstellen (statt `string`-Key)
- [ ] `TextInputMsg` (post-IME) als separaten Typ neben `CharMsg` einführen
- [ ] `MouseMsg` um `MouseEnter`/`MouseLeave` Action erweitern
- [ ] `ScrollMsg` um `Precise bool` (Trackpad vs. Mausrad) und `Pos` erweitern
- [ ] `TouchMsg` implementieren (TouchID, Phase, Force)
- **Abhängig von:** nichts (Kern-Package `input/`, keine Imports)

### 0.2 RenderCtx.Events — Input-Dispatch an Widgets (RFC-001 §4, RFC-002 §2.6)
- [ ] `RenderCtx` um `Events []InputEvent` Feld erweitern
- [ ] `InputEvent` als typisierten Union-Wrapper implementieren
- [ ] Framework-internes Dispatching: Mouse → Hit-Test → Widget, Keyboard → Focus → Widget
- **Abhängig von:** 0.1

### 0.3 Focus-Management (RFC-002 §2.3)
- [ ] `Focusable`-Interface auf `Widget` definieren (`FocusOptions() FocusOpts`)
- [ ] Tab-Reihenfolge aus Layout-Baum ableiten
- [ ] `FocusGainedMsg` / `FocusLostMsg` implementieren
- [ ] `RequestFocusMsg` / `ReleaseFocusMsg` implementieren
...
- [ ] `Animator`-Interface auf `WidgetState` definieren: `Tick(dt) bool`
- [ ] Animation-Pass im App-Loop vor Reconcile einfügen
- [ ] Automatisches Dirty-Marking bei `Tick() == true`
- **Abhängig von:** nichts (nutzt existierendes `anim/` Package)

### 0.5 `Anim[T]` Interpolatable erweitern (RFC-002 §1.4)
- [ ] `Interpolatable` Constraint um `draw.Color`, `draw.Point`, `draw.Size`, `draw.Rect`, `draw.CornerRadii` erweitern
- [ ] Alternativ: `Lerper[T]`-Pattern um Zyklus `anim/ → draw/` zu vermeiden
- **Abhängig von:** nichts
...
- [ ] SDF-Atlas auf Multi-Channel SDF (MSDF) umstellen
- [ ] Schärfere Kurven bei kleinen Schriftgrößen
- **Abhängig von:** nichts
